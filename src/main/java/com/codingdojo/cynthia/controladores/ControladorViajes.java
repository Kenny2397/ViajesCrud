...
public class ControladorViajes {
    ...
    public String edit(@PathVariable("id") Long id, Model model,@ModelAttribute("gasto") Gasto gasto) {
        ...
        System.out.println(gasto_edit.getExpense_name());
        model.addAttribute("gasto", gasto_edit);
        return "edit.jsp";
    }
	
	@RequestMapping(value="/edit/{id_url}", method=RequestMethod.PUT)
	public String update(@PathVariable("id_url") Long id, @Valid @ModelAttribute("gasto") Gasto gasto, BindingResult result) {
		if(result.hasErrors()) {
			return "edit.jsp";
		} else {
			servicio.save_gasto(gasto);
	...
	public String show(@PathVariable(name="id") Long id,
					   Model model) {
		Gasto thisGasto = servicio.find_gasto(id);
		model.addAttribute("gasto", thisGasto);
		return "show.jsp";
	}
